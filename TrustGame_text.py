...
#imgdir = '%s/Images/%s/%s_friend.png' % (expdir, subj_id, subj_id)
...
def do_run(run, trials):
    ...
    trials.saveAsWideText(fileName=log_file.format(subj_id, subj_id, run+1), delim=',', appendFile=True)
...
if len (specific_run)==1:
    ...
    for run_idx, trials in enumerate([trials_run1, trials_run2, trials_run3, trials_run4, trials_run5, trials_run6]):
